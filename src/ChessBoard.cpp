...
void ChessBoard::fillBoard()
{
    ...
    //Pawns
    for (int i = 0; i < m; i++)
        arr[1][i]->setFigure(new Pawn(ChessFigure::BLACK));
    for (int i = 0; i < m; i++)
        arr[n - 2][i]->setFigure(new Pawn(ChessFigure::WHITE));
    for (int i = 1; i < queenColumn - 2; i++)
        arr[0][i]->setFigure(new Pawn(ChessFigure::BLACK));
    for (int i = 1; i < queenColumn - 2; i++)
        arr[n - 1][i]->setFigure(new Pawn(ChessFigure::WHITE));
...
void ChessBoard::buttonClicked(ChessTile *tile)
{
    ...
    {
        if (tile->getFigure() == nullptr)
            return;
        if (tile->getFigure()->getTeam() == prevTeam)
            return;
        lastPressed = tile;
